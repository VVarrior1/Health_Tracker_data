...
const HeartRateChart: React.FC<HeartRateChartProps> = ({ 
  ...
  // Calculate domain for Y-axis to prevent cutoff
  const maxHeartRate = useMemo(() => {
    const maxValue = Math.max(
      ...filteredHeartRate.map(item => item.value),
      ...filteredRestingHeartRate.map(item => item.value)
    );
    // Add 10% padding to the top to prevent cutoff
    return Math.ceil(maxValue * 1.1);
  }, [filteredHeartRate, filteredRestingHeartRate]);
